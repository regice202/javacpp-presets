...
case $PLATFORM in
    ...
    windows-x86_64)
        ...
        cd libfreenect2-$LIBFREENECT2_VERSION
        CC="gcc -m64" CXX="g++ -m64" LDFLAGS="-L$INSTALL_PATH/lib/" $CMAKE -G "MSYS Makefiles" -DCMAKE_BUILD_TYPE=Release -DBUILD_EXAMPLES=OFF -DBUILD_OPENNI_DRIVER=OFF -DENABLE_CUDA=ON -DENABLE_CXX11=OFF -DCUDA_TOOLKIT_ROOT_DIR="%ProgramFiles%/NVIDIA GPU Computing Toolkit/CUDA/v$CUDA_VERSION" -DENABLE_OPENCL=OFF -DENABLE_VAAPI=OFF -DENABLE_TEGRAJPEG=OFF -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DLibUSB_LIBRARIES=$INSTALL_PATH/bin/libusb-1.0.dll -DGLFW3_LIBRARY=$INSTALL_PATH/bin/glfw3.dll -DTurboJPEG_LIBRARIES=$INSTALL_PATH/bin/turbojpeg.dll .
